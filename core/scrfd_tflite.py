...
class SCRFDTDMMTFLitePostModel(tf.keras.Model):

    def __init__(self, tdmm_cfg, interpreter, n_objs, top_k_n, kp_thres,
                 nms_iou_thres, resize_shape, *args, **kwargs):
        super(SCRFDTDMMTFLitePostModel, self).__init__(*args, **kwargs)
        ...
        self.multi_lvs_keys = [
            [
                "stride_8_cls", "stride_8_bbox", "stride_8_params",
                "stride_8_trans"
            ],
            [
                "stride_16_cls", "stride_16_bbox", "stride_16_params",
                "stride_16_trans"
            ],
            [
                "stride_32_cls", "stride_32_bbox", "stride_32_params",
                "stride_32_trans"
            ],
        ]
    ...
    def call(self, x, training=False):
        ...
        serving_default = self.interpreter.get_signature_runner(
            'serving_default')
        signature_dict = serving_default.__dict__
        outputs = serving_default(image_inputs=imgs)
        multi_lv_feats, temp = [], []
        pred_branches = {}
        for map_key, key in zip(self.map_keys, outputs.keys()):
            pred_branches[map_key] = outputs[key]
        multi_lv_feats = []
    ...
    def _anchor_assign(self, batch_size, multi_lv_feats):
        ...
        obj_start_idx = 0
        b_idx_list, b_bbox_list, b_lnmk_list, b_kpss_list = [], [], [], []
        for i, (lv_feats,
