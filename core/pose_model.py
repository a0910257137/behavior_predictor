...
class PosePostModel(tf.keras.Model):
    ...
    def call(self, x, training=False):
        ...
        b_bboxes, b_poses = self._obj_detect(batch_size, preds["obj_heat_map"],
                                             preds['obj_size_maps'],
                                             preds['obj_pose_map'])
        return b_bboxes, b_poses

    # @tf.function
    def _obj_detect(self, batch_size, hms, sms, pms):
        ...
        b_mask = ~tf.math.reduce_any(tf.math.is_inf(b_bboxes), axis=-1)
        b_poses = self._pose(batch_size,
                             tf.reshape(b_bboxes[b_mask], (batch_size, -1, 6)),
                             pms)

        return b_bboxes, b_poses

    # @tf.function
    def _pose(self, batch_size, b_bboxes, pms):
        b_bboxes = b_bboxes[..., :4]
        b_kps = (b_bboxes[..., :2] + b_bboxes[..., 2:]) / 2
        b_kps = tf.einsum('b n d , b d -> b n d', b_kps, 1 / self.resize_ratio)
        b_kps = tf.cast(b_kps, tf.int32)
        _, n, d = [tf.shape(b_kps)[i] for i in range(3)]
        b_idx = tf.tile(
            tf.range(batch_size, dtype=tf.int32)[:, None, None], [1, n, 1])
        b_idx = tf.concat([b_idx, b_kps], axis=-1)
        b_poses = tf.gather_nd(pms, b_idx)
        return b_poses
