...
class SCRFDTDMMOptPostModel(tf.keras.Model):
    ...
    def __init__(self, tdmm_cfg, weight_root_dir, pred_model, n_objs, top_k_n,
        ...
        self.exp_base = tf.cast(head_model['expPC'], tf.float32)[:, :self.n_exp]
    ...
    def _anchor_assign(self, batch_size, multi_lv_feats):
        ...
        for i, (lv_feats,
            ...
            b_param_preds = tf.reshape(b_param_preds,
                                       (N // 2, 2, tf.shape(b_param_preds)[-1]))
            ...
            vertices = self.u_base[None, ...] + tf.linalg.matmul(
                self.shp_base, b_param_preds[:, self.n_R:self.n_R + self.n_shp,
                                             None]
            ) + tf.linalg.matmul(self.exp_base,
                                 b_param_preds[:, self.n_R + self.n_shp:, None])
        ...
        b_lnmk_outputs = tf.tensor_scatter_nd_update(b_lnmk_outputs, b_idx_list,
                                                     b_lnmk_list)
    ...
    def _post_convolution(self, stride_i, feat_map_idxs, reg_feat, x):
        ...
        b_bbox_preds *= bbox_scale
        # self.param_weighs_conv3x3, self.param_biass_conv3x3, self.param_weights_pred, self.param_biass_pred
        # -------------------------------------------------- params branch
